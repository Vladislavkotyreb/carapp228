...
struct IssuesScreen: View {
    ...
    private func issues(from diagnosis: Diagnosis) -> [EngineIssue] {
        ...
        // Каскад чистки не выделил ни одного механического куска — запись
        // разбирали целиком, и ненадёжен тогда **весь** ответ, а не только
        // список деталей. Проверено на трёх записях: пять секунд тишины дают
        // «неисправность 73 %» и «выхлоп 100 %», чистый синус — «неисправность
        // 92 %». Голова причин обязана разложить свои 100 % по деталям, класса
        // «ничего» у неё нет, а голова «есть ли поломка» на таком входе просто
        // вне своей области. Показывать из этого нечего.
        guard diagnosis.segmentCount > 0 else {
            return [EngineIssue(
                title: "Чистого звука двигателя не выделено",
                detail: "Разбирать нечего: в записи нет механического звука. "
                        + "Запишите ближе к работающему мотору, без разговоров.")]
        }
