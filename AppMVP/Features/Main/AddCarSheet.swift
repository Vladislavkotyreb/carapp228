                ...
                VStack(spacing: 32) {
                    ...
                    VStack(spacing: 24) {
                        if tab == 0 {
                            ...
                            // Фото — только здесь. Под «По номеру» его тоже
                            // выносили, потому что `confirmFoundCar` снимок
                            // читает, а дойти до этой ветки с главной было
                            // невозможно. Рассуждение было обратным: ветка
                            // недостижима не потому, что забыли кнопку, а
                            // потому что по номеру прикреплять нечего.
                            // Поиск возвращает поколение и кадр каталога,
                            // а `car.photo` этот кадр перебивает — и вдобавок
                            // переключает шапку главной в другой режим
                            // (`photoMix`: свой снимок во весь верх против
                            // студийного ассета в блоке макета). Свой снимок
                            // закрывает дыру там, где поиска нет вовсе.
                            photoBlock
                        }
                    }
                }
