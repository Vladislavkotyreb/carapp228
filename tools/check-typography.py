...
WARNINGS = [
    ...
    # Пробелы здесь именно обычные, а не \s: после `collapse()` \u{00A0}
    # становится настоящим неразрывным пробелом, а Python считает его пробельным
    # символом — и правило ругалось на строки, где перенос уже запрещён. То есть
    # ровно на те, что написаны верно.
    ("однобуквенный предлог или союз может остаться в конце строки",
     r"(?<=[ ])[вксоуиая](?=[ ])", rf"неразрывный пробел после него: \\u{{00A0}}"),
]
