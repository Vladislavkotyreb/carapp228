...
WARNINGS = [
    ...
    # Справа именно обычный пробел, а не `\s`. С `\s` правило само себя не
    # давало исполнить: в Python `\s` покрывает и неразрывный пробел, поэтому
    # строка, починенная ровно так, как советует подсказка, оставалась
    # нарушением. Отсюда и тянулся хвост предупреждений, который никто не мог
    # снять. Слева `\s` уместен: там подойдёт любой разделитель.
    ("однобуквенный предлог или союз может остаться в конце строки",
     "(?<=\\s)[вксоуиая](?=[ \t])", rf"неразрывный пробел после него: \\u{{00A0}}"),
]
