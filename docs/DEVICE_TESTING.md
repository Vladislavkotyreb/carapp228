...
онбординг → добавление авто → главный экран. Состояние раздельное: пройден ли
онбординг — флаг `hasCompletedOnboarding` в `UserDefaults`, а есть ли машина —
запрос к SwiftData (`@Query private var cars`), флага `hasAddedCar` больше нет
(`AppMVP/Navigation/AppState.swift`, [`DECISIONS.md`](DECISIONS.md)). Поэтому
для повтора с нуля приложение надо удалить с телефона: сносить надо и флаг,
и базу, а удаление приложения уносит и то, и другое. Удалить одну машину внутри
приложения — вернуться к экрану добавления, но не к онбордингу.
